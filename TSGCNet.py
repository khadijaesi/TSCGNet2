...
class TSGCNet(nn.Module):
    def __init__(self, k=16, in_channels=12, output_channels=15):
    ...
    def forward(self, x):
        ...
        #print("coor concat :" ,coor.shape)
        coor = self.conv4_c(coor)
        #print("coor :" ,coor.shape)

        nor = torch.cat((nor1, nor2, nor3), dim=1)
        #print("nor concat :" ,nor.shape)
        nor = self.conv4_n(nor)
        #print("nor :" ,nor.shape)

        avgSum_coor = coor.sum(1)/512
        #print("avgSum_coor :" , avgSum_coor.shape)
        avgSum_nor = nor.sum(1)/512
        #print("avgSum_nor :" ,avgSum_nor.shape)
        avgSum = avgSum_coor+avgSum_nor
        #print("avgSum :" ,avgSum.shape)

        weight_coor = (avgSum_coor / avgSum).reshape(x.size(0), 1, 12000)

        #print("avgSum_coor / avgSum :" ,(avgSum_coor / avgSum).shape)
        #print("weight_coor :" ,weight_coor.shape)

        weight_nor = (avgSum_nor / avgSum).reshape(x.size(0), 1, 12000)
        #print("avgSum_nor / avgSum :" ,(avgSum_nor / avgSum).shape)
        #print("weight_nor :" ,weight_nor.shape)

        x = torch.cat((coor*weight_coor, nor*weight_nor), dim=1)
        #print("x :" ,x.shape)
        
        #x = torch.cat((coor.unsqueeze(3) * weight_coor.unsqueeze(2), nor.unsqueeze(3) * weight_nor.unsqueeze(2)), dim=1)
        #x = x.permute(0, 3, 2, 1).reshape(1, 24000, 15)


        weight = self.fa(x)
        #print("weight:" ,weight.shape)
        x = weight*x
        #print("x*weight:" ,x.shape)

        x = self.pred1(x)
        #print("x_pred1:" , x.shape)
        self.dp1(x)
        #print("x_pred1v2:" , x.shape)

        x = self.pred2(x)
        #print("x_pred2:" , x.shape)
        self.dp2(x)
        #print("x_pred2v2:" , x.shape)

        x = self.pred3(x)
        #print("x_pred3:" , x.shape)
        self.dp3(x)
        #print("x_pred3v2:" , x.shape)

        score = self.pred4(x)
        #print("score xpred4:" , score.shape)
...
if __name__ == "__main__":
    ...
    x = torch.rand(2,24,12000)
    x = x.cuda()
    model = TSGCNet(in_channels=12, output_channels=15, k=8)
    model = model.cuda()
    y = model(x)
    print(y.shape)
