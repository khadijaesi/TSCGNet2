...
import random
from data_augmentation import rotate_mesh
...
if __name__ == "__main__":
    ...
    """-------------------------------- Dataloader --------------------------------"""
    train_dataset = plydataset("/content/TSGCNet/data/train")
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True , num_workers=4)
    test_dataset = plydataset("/content/TSGCNet/data/test")
    ...
    for epoch in range(0, 200):
        ...
        for i, data in tqdm(enumerate(train_loader, 0), total=len(train_loader), smoothing=0.9):
            ...
            for rotation_index in range(num_rotation) :
              rotation_angles = [random.uniform(0,2*np.pi) for _ in range(3)] 
              points_face_rotated = rotate_mesh(points_face , rotation_angles)
              #print('points_face_rotated' ,points_face_rotated.shape)
              points_face_rotated = torch.from_numpy(points_face_rotated)
              coordinate = points_face_rotated.transpose(2,1)
              #print('coordinate train' ,coordinate.shape)
              #print('============================================')
        ...
        if epoch % 10 == 0:
            ...
            metrics, mIoU, cat_iou = test_semseg(model, test_loader, num_classes=15)
